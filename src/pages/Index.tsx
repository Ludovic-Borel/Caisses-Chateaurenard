...
export default function Index() {
  ...
  useEffect(() => {
    ...
    return () => { unsubMonth(); unsubDrivers(); };
  ...
  useEffect(() => {
    ...
    return () => { if (excelBackupTimeoutRef.current) clearTimeout(excelBackupTimeoutRef.current); };
  ...
  const handleSyncToSupabase = useCallback(async () => {
    ...
    try {
      ...
      if (result.errors.length > 0) {
        toast.error(`Sync terminé avec ${result.errors.length} erreur(s) : ${result.errors.slice(0, 3).join("; ")}${result.errors.length > 3 ? `... (${result.errors.length - 3} autres)` : ""}`, { duration: 8000 });
      } else {
        toast.success(`Synchronisé ! ${result.monthsMigrated}/${result.monthsTotal} mois et ${result.driversCount} conducteurs envoyés à Supabase`);
      }
    } catch (e: any) {
  ...
  const handleConfigureSupabase = useCallback(async () => {
    try {
      ...
      } else { toast.error("Impossible d'initialiser Supabase"); }
    } catch (e: any) { toast.error(`Erreur configuration Supabase: ${e?.message || String(e)}`); }
  }, []);

  useEffect(() => {
    if (!isSupabaseConfigured()) return;
    const id = setInterval(async () => {
      try { const res = await migrateLocalToRemote(); if (res.errors.length > 0) console.warn("Auto sync errors:", res.errors); } catch (e) { console.warn("Auto sync failed:", e); }
    }, 5 * 60 * 1000);
  ...
  useEffect(() => {
    const onOnline = async () => {
      if (!isSupabaseConfigured()) return;
      try { const res = await migrateLocalToRemote(); if (res.errors.length === 0) toast.success("Sync automatique terminée"); } catch (e) { console.warn("Online sync failed:", e); }
    };
  ...
  const handleDriverDataChange = useCallback((driver: string, driverData: DriverMonthData) => {
    setData((prev) => ({ ...prev, drivers: { ...prev.drivers, [driver]: driverData } }));
  }, []);
  ...
  const handleRemoveDriver = useCallback(async (name: string) => {
    ...
    try {
      ...
      await Promise.all(future.filter((m) => m.drivers && m.drivers[name]).map((m) => { const { [name]: _, ...rest } = m.drivers; return saveMonth({ ...m, drivers: rest }); }));
    } catch (e) { console.warn("Failed to strip driver from future months", e); }
  ...
  const handleExtractionImport = useCallback(async (files: FileList | null) => {
    ...
    try {
      ...
      if (result.year !== dataRef.current.year || result.month !== dataRef.current.month) {
        toast.error(`Le fichier concerne ${MONTH_NAMES[result.month]} ${result.year}, mois courant : ${MONTH_NAMES[dataRef.current.month]} ${dataRef.current.year}`);
        setLoading(false);
      ...
      for (const n of knownNames) { const p = parseAppDriverName(n); if (!p.lastName) continue; const arr = lastNameIndex.get(p.lastName) || []; arr.push({ canonical: n, initial: p.initial }); lastNameIndex.set(p.lastName, arr); }
      ...
      for (const [normFull, dayMap] of Object.entries(result.byDriver)) {
        ...
        if (candidates && candidates.length === 1) { canonical = candidates[0].canonical; }
        else if (candidates && candidates.length > 1) {
          const exact = candidates.find((c) => c.initial && parsed.initial && c.initial === parsed.initial);
          if (exact) canonical = exact.canonical;
          else { const noInit = candidates.find((c) => !c.initial); if (noInit) canonical = noInit.canonical; }
        }
        if (!canonical) {
          let total = 0; const days = Object.keys(dayMap).length;
          for (const dd of Object.values(dayMap)) for (const v of Object.values(dd)) total += v;
          unmatched.push({ name: normFull, days, total: Math.round(total * 100) / 100 }); continue;
        }
    ...
    } catch (e) { const msg = e instanceof Error ? e.message : String(e); toast.error(`Import extraction : ${msg}`); }
    finally { setLoading(false); if (extractionFileRef.current) extractionFileRef.current.value = ""; }
  }, []);
  ...
  const handleImportFiles = useCallback(async (files: FileList | null) => {
    ...
    for (let idx = 0; idx < fileArray.length; idx++) {
      ...
      try {
        ...
        const merged: MonthData = existing ? { ...existing, drivers: { ...existing.drivers, ...Object.fromEntries(Object.entries(result.data.drivers).map(([driver, importedData]) => { const existingDriver = existing.drivers[driver] || { days: {} }; return [driver, { days: { ...existingDriver.days, ...importedData.days }, ...(importedData.notReturned && { notReturned: importedData.notReturned }), ...(existingDriver.extracts && { extracts: existingDriver.extracts }) }]; })) } } : result.data;
    ...
    if (allNewDrivers.size > 0) { const currentSet = new Set(driversRef.current); const toAdd = [...allNewDrivers].filter((d) => !currentSet.has(d)); if (toAdd.length > 0) { const updated = [...driversRef.current, ...toAdd].sort(); setDrivers(updated); await saveDrivers(updated); } }
    if (lastImported && lastImported.year === dataRef.current.year && lastImported.month === dataRef.current.month) { const refreshed = await loadMonth(lastImported.year, lastImported.month); if (refreshed) { skipNextSave.current = true; setData(refreshed); } }
  ...
  const handleSelectBackupDir = useCallback(async () => { const result = await selectBackupDir(); if (result) { setBackupDirName(result.name); toast.success(`Dossier de sauvegarde : ${result.name}`); } }, []);
  const handleClearBackupDir = useCallback(async () => { await clearBackupDir(); setBackupDirName(null); toast.success("Dossier de sauvegarde retiré"); }, []);
  const handleSelectTemplateFile = useCallback(async () => { const result = await selectTemplateFile(); if (result) { setTemplateFileName(result.name); toast.success(`Fichier modèle : ${result.name}`); } }, []);
  const handleClearTemplateFile = useCallback(async () => { await clearTemplateFile(); setTemplateFileName(null); toast.success("Fichier modèle retiré"); }, []);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "i") { e.preventDefault(); if (isCurrentMonth && !extractionMode) fileInputRef.current?.click(); }
      if ((e.ctrlKey || e.metaKey) && e.key === "z") { if (undoState) { e.preventDefault(); handleUndo(); } }
    };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-50">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-72">
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); fileInputRef.current?.click(); }} className="cursor-pointer">
                  <Upload className="h-4 w-4 mr-2" /><span>Importer Excel</span>
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); if (!extractionMode) { toast.error("Activer le mode Extraction pour importer une extraction"); return; } extractionFileRef.current?.click(); }} className={`cursor-pointer ${!extractionMode ? "opacity-40" : ""}`}>
                  <FileDown className="h-4 w-4 mr-2" /><span>Importer Extraction</span>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); handleSelectBackupDir(); }} className="cursor-pointer">
                  <Folder className="h-4 w-4 mr-2" /><div className="flex flex-col"><span className="text-sm">Dossier de sauvegarde</span><span className="text-[11px] text-muted-foreground truncate max-w-[200px]">{backupDirName || "Non défini"}</span></div>
                </DropdownMenuItem>
                {backupDirName && <DropdownMenuItem onSelect={handleClearBackupDir} className="cursor-pointer text-destructive"><Folder className="h-4 w-4 mr-2" /><span>Retirer le dossier</span></DropdownMenuItem>}
                <DropdownMenuSeparator />
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); handleSelectTemplateFile(); }} className="cursor-pointer">
                  <FileText className="h-4 w-4 mr-2" /><div className="flex flex-col"><span className="text-sm">Fichier modèle vierge</span><span className="text-[11px] text-muted-foreground truncate max-w-[200px]">{templateFileName || "Aucun (export normal)"}</span></div>
                </DropdownMenuItem>
                {templateFileName && <DropdownMenuItem onSelect={handleClearTemplateFile} className="cursor-pointer text-destructive"><FileText className="h-4 w-4 mr-2" /><span>Retirer le modèle</span></DropdownMenuItem>}
                <DropdownMenuSeparator />
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); handleConfigureSupabase(); }} className="cursor-pointer">
                  <Database className="h-4 w-4 mr-2" /><span>Configurer Supabase</span>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                ...
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setHelpOpen(true); }} className="cursor-pointer">
                  <HelpCircle className="h-4 w-4 mr-2" /><span>Aide</span>
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); window.open("https://github.com/Ludovic-Borel/Caisses-Chateaurenard/issues/new", "_blank"); }} className="cursor-pointer">
                  <Bug className="h-4 w-4 mr-2" /><span>Signaler un bug</span>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            {undoState && (
              <Button variant="destructive" size="sm" className="text-xs h-8 no-print" onClick={handleUndo} title="Annuler le dernier import">
                <RotateCcw className="h-3.5 w-3.5 mr-1" /> Annuler
              </Button>
            )}
            <Button variant="secondary" size="sm" className="text-xs h-8 no-print hidden md:inline-flex" onClick={handlePrint} title="Exporter en PDF (impression navigateur)">
        ...
        <div className="flex items-center flex-wrap gap-3">
          ...
          <Button variant={selectedDriver === null ? "default" : "outline"} onClick={() => { setExtractionMode(false); setSelectedDriver(null); }}>
            <TableProperties className="h-4 w-4 mr-2" /> Récap global
          </Button>
          <Button variant={selectedDriver === "__yearly__" ? "default" : "outline"} onClick={() => { setExtractionMode(false); setSelectedDriver("__yearly__"); }}>
        ...
        {/* Indicateurs statut + thème + Supabase + Extraction */}
        <div className="flex items-center flex-wrap gap-3">
          {/* Indicateurs + icône thème */}
          <div className="flex items-center gap-2">
            ...
            {importProgress && (
              <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary rounded-full transition-all duration-300" style={{ width: `${(importProgress.current / importProgress.total) * 100}%` }} />
                </div>
              </span>
            )}
            {modifiedSinceSave && (
            ...
            <Button variant="ghost" size="sm" className="text-xs h-8" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title={theme === "dark" ? "Mode clair" : "Mode sombre"}>
          ...
          {!extractionMode && supabaseStatus && (
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium cursor-pointer ${supabaseStatus.connected && supabaseStatus.monthsTable && supabaseStatus.driversTable ? "bg-green-100 text-green-800 border border-green-300" : "bg-red-100 text-red-800 border border-red-300"}`}
              onClick={handleSyncToSupabase}
              title={supabaseStatus.error ? `Erreur: ${supabaseStatus.error}` : `${supabaseStatus.monthsCount} mois, ${supabaseStatus.driversCount} conducteurs`}
            >
              <Database className="h-3 w-3" />
              <span>{syncing ? "Sync..." : supabaseStatus.connected && supabaseStatus.monthsTable && supabaseStatus.driversTable ? `${supabaseStatus.monthsCount}M ${supabaseStatus.driversCount}C` : "Déconnecté"}</span>
            </div>
          )}
          {!extractionMode && !supabaseStatus?.connected && (
            <Button variant="outline" size="sm" className="text-xs h-8" onClick={handleSyncToSupabase} disabled={syncing}>
              <Database className="h-3.5 w-3.5 mr-1" />{syncing ? "Sync..." : "Sync Supabase"}
            </Button>
          )}

          {/* Bouton Extraction */}
          <Button variant={extractionMode ? "default" : "outline"} onClick={() => {
            if (extractionMode) { setExtractionMode(false); return; }
          ...
          {extractionMode && (
            <input ref={extractionFileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={(e) => handleExtractionImport(e.target.files)} />
          )}
        ...
        <div className="flex gap-6">
          {sidebarOpen && (
            <div className="w-[220px] flex-shrink-0">
              <DriverList drivers={Array.from(new Set([...drivers, ...Object.keys(data.drivers || {})])).sort()}
                activeDrivers={drivers} selectedDriver={selectedDriver}
                onSelect={(d) => { if (d === "__stats__" || d === "__dashboard__" || d === "__yearly__" || d === "__comparison__" || d === null) setExtractionMode(false); setSelectedDriver(d); }}
                onAddDriver={handleAddDriver} onRemoveDriver={handleRemoveDriver} onRenameDriver={handleRenameDriver}
              />
            </div>
          )}
            ...
            <div className="bg-card rounded-lg border border-border shadow-sm p-4">
              <div className="flex items-center justify-end mb-2">
                <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground"
                  onClick={() => { const next = !sidebarOpen; setSidebarOpen(next); localStorage.setItem("recettes_sidebar_open", String(next)); }}
      ...
      <footer className="text-center text-[10px] text-muted-foreground/70 py-2 select-none border-t border-border/20 no-print">
        ...
        {newVersion && <span className="ml-2 text-amber-500 font-semibold">• Nouvelle version disponible : <a href="https://github.com/Ludovic-Borel/Caisses-Chateaurenard/releases" target="_blank" rel="noopener noreferrer" className="hover:text-primary underline">{newVersion}</a></span>}
      </footer>
      ...
      {helpOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setHelpOpen(false)}>
          <div className="bg-card text-card-foreground rounded-lg shadow-xl border border-border max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-border"><h2 className="text-lg font-bold">Aide - Caisses Chateaurenard</h2></div>
            <div className="p-4 text-sm space-y-3">
              <div><h3 className="font-semibold text-primary mb-1">📥 Importer un fichier Excel</h3><p className="text-muted-foreground">Ouvrez le menu <strong>Config et Import</strong> → <strong>Importer Excel</strong>. Sélectionnez un fichier "Recettes Lignes MM - YYYY.xlsx". Les données sont fusionnées avec le mois correspondant.</p></div>
              <div><h3 className="font-semibold text-primary mb-1">📄 Importer une Extraction</h3><p className="text-muted-foreground">Activez d'abord le mode <strong>Extraction</strong> (bouton en haut), puis utilisez <strong>Importer Extraction</strong> dans le menu. Les fichiers attendus sont du type <em>ventes-realisees-orientees-reseau_*.xlsx</em>.</p></div>
              <div><h3 className="font-semibold text-primary mb-1">💾 Sauvegarde automatique</h3><p className="text-muted-foreground">Configurez un <strong>dossier de sauvegarde</strong> dans le menu. Un fichier Excel est automatiquement généré à chaque modification (5s après la dernière saisie).</p></div>
              <div><h3 className="font-semibold text-primary mb-1">☁️ Synchronisation Cloud</h3><p className="text-muted-foreground">Cliquez sur <strong>Configurer Supabase</strong> pour entrer votre URL et clé API. Une fois connecté, les données sont synchronisées automatiquement toutes les 5 minutes et au retour en ligne.</p></div>
              <div><h3 className="font-semibold text-primary mb-1">🖨️ Export PDF</h3><p className="text-muted-foreground">Utilisez le bouton <strong>PDF</strong> dans le header. L'impression navigateur génère un PDF en format A4 paysage.</p></div>
              <div><h3 className="font-semibold text-primary mb-1">🌙 Mode sombre</h3><p className="text-muted-foreground">Cliquez sur l'icône ☀️/🌙 à côté du badge Supabase pour basculer entre les thèmes clair et sombre.</p></div>
            </div>
            <div className="p-4 border-t border-border flex justify-end"><Button variant="default" size="sm" onClick={() => setHelpOpen(false)}>Fermer</Button></div>
          </div>
        </div>
      )}
    </div>
